...
if(isset($_POST['edit']))
{
    $pk = $crypt->decryptId($_POST['TypeID']); 
$result=$typeObj->selectbypk($pk);
...
{
   $typeObj->TypeName=$_POST['TypeName'];
   $typeObj->TypeDescription=$_POST['TypeDescription'];
...
//    $typeObj->ImageDescription=$_POST['ImageDescription'];
   $typeObj->TypeUrl=$_POST['TypeUrl'];
   $typeObj->sequence=$_POST["sequence"]; 

   $typeObj->MetaTitle = $_POST['MetaTitle'];
   $typeObj->MetaKeywords = $_POST['MetaKeywords'];
   $typeObj->MetaDescription = $_POST['MetaDescription'];
   $typeObj->MetaOgTitle = $_POST['MetaOgTitle'];
   $imgname2 = $_FILES['MetaOgImage']['name'];
   $file_tmp2 = $_FILES['MetaOgImage']['tmp_name'];
   move_uploaded_file($file_tmp2, '../../Images/type/' . $imgname2);
   $typeObj->MetaOgImage=$_FILES['MetaOgImage']['name'];
   $typeObj->MetaOgDescription = $_POST['MetaOgDescription'];
   $typeObj->MetaOgUrl = $_POST['MetaOgUrl'];
   $typeObj->UserID = $_SESSION["UserID"];
   $typeObj->IsActive =  $_POST['IsActive'];
   
   $result=$typeObj->insertData();
   if ($result) {
    ...
    <script>
        Swal.fire({
            ...
            window.location = 'Type';  // Replace with the appropriate redirect URL
        });
    </script>";
} else {
    echo "<script src='https://cdn.jsdelivr.net/npm/sweetalert2@11.0.18'></script>
...
//    $typeObj->ImageDescription=$_POST['ImageDescription'];
   ...
   $imgname2 = $_FILES['MetaOgImage']['name'];
    ...
    if($imgname2 != '') 
    {
      $file_tmp2 = $_FILES['MetaOgImage']['tmp_name'];
      move_uploaded_file($file_tmp2, '../../Images/type/' . $imgname2);
      unlink('../../Images/type/' . $oldimgname2);
      $typeObj->MetaOgImage=$_FILES['MetaOgImage']['name'];
    } 
    else
    {
      $typeObj->MetaOgImage=$_POST['Old_ogImage'];
    }  
   $typeObj->MetaOgDescription = $_POST['MetaOgDescription'];
   $typeObj->MetaOgUrl = $_POST['MetaOgUrl'];
   $typeObj->UserID = $_SESSION["UserID"];
   $typeObj->IsActive =  $_POST['IsActive'];
   $result=$typeObj->updateData($_POST['TypeID']);
   if ($result) {
    echo "<script src='https://cdn.jsdelivr.net/npm/sweetalert2@11.0.18'></script>
    <script>
        Swal.fire({
...
<div class="content d-flex flex-column flex-column-fluid" id="kt_content">
                        ...
                        <form action="typeaddedit.php" method="post" enctype="multipart/form-data">
                            <!-- main cards -->
                                ...
                                <div id="kt_content_container" class="container-xxl">
                                    ...
                                    <div class="card card-flush shadow-sm ">
                                        ...
                                        <div class="card-header border-1">
                                            <h3 class="card-title text-danger " style="font-weight: 900;">type Details
                                            </h3>
                                        </div>
                                        ...
                                        <div class="card-body py-5 font-weight-bolder">
                                            ...
                                            <label class="required  form-label  font-weight-bolder "
                                                style="font-weight: 900;"> type Name </label>
                                    
                                            <input type="text" name="TypeName" class="form-control mb-2"
                                                placeholder="type Title" value="<?php echo $selectbypk['TypeName']  ?>" required>
                                    
                                        </div>
                                        <br />
                                        <div>
                                            <label class="required  form-label  font-weight-bolder "
                                                style="font-weight: 900;">type Url:</label>
                                    
                                                <input type="text" name="TypeUrl" class="form-control mb-2"
                                                placeholder="type Url" value="<?php echo $selectbypk['TypeUrl']  ?>" required>
                                        </div>
                                       
                                        <br />
                                        <div>
                                            <label class="  form-label  font-weight-bolder "
                                                style="font-weight: 900;">type Descriptiion:</label>
                                    
                                                <textarea  id="editor" name="TypeDescription" class="form-control mb-2"
                                                placeholder="type Descriptiion"  ><?php echo $selectbypk['TypeDescription'];  ?></textarea>
...
<script>
    ...
    var firstField = document.querySelector('input[name="TypeName"]');
    var secondField = document.querySelector('input[name="TypeUrl"]');
    ...
    function validateImageSize() {
            ...
            image.onload = function () {
                if (this.width !== 1200 || this.height !== 600) {
                    ...
                    cancelButton.click(); // Clear the file input to prevent submitting invalid image
                                                    ...
                                                    <!--end::Cancel-->
                                                    <!--begin::Remove-->
                                                    <span  id="cenclik" class="btn btn-icon btn-circle btn-active-color-primary w-25px h-25px bg-body shadow" data-kt-image-input-action="remove" data-bs-toggle="tooltip" title="" data-bs-original-title="Remove avatar">
                                                        <i class="bi bi-x fs-2"></i>
                                                    </span>
                                                    <!--end::Remove-->
                                                </div>
                                                ...
                                                <!--end::Card title-->
                                                       ...
                                                       <img class=" w-200px h-200px" src="../../Images/type/<?php echo $selectbypk['MetaOgImage'] ?>"
                                    ...
                                    <div align="center">
                                        <div class="row">
                                            <div class="col mb-4">
                                            <input type="hidden" name="TypeID" class="form-control mb-2"
                                                value="<?php echo  $selectbypk['TypeID']  ?>">
        ...
        if(isset($_POST['edit'])) {
          ...
          echo "insert";
                                                ...
                                                <a href="type.php" class="btn btn-danger">CANCLE</a>
                                            </div>
                                        </div>
                                    </div>
